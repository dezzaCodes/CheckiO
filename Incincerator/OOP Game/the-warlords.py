...
class Army:
	...
	def contains_unit(self, cls):
		return [unit for unit in self.units if isinstance(unit, cls) and unit.is_alive]

	def move_units(self):
		if self.contains_unit(Warlord):
			lancers = self.contains_unit(Lancer)
			healers = self.contains_unit(Healer)
			warlord = self.contains_unit(Warlord)
			other_attackers = [u for u in self.units if
							   u.attack > 0 and not isinstance(u, Lancer) and not isinstance(u, Warlord)]

	comp = lancers + other_attackers

	self.units = comp[:1] + healers + comp[1:] + warlord[:1]
